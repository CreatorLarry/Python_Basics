class Account:
    ...
    def deposit(self, amount):
        ...
        print(f"Amount ${amount} has been deposited successfully to account {self.acc_num}. New balance  is ${self.balance}") # Another way of printing by using format

    # Withdrawing
    def withdraw(self, amount):
        if self.balance < amount:
            print(f"Your balance has Insufficient Funds. Balance is ${self.balance}. To access loans, press 1 to continue.")

        else:
            self.balance = self.balance - amount
            print(f"Amount ${amount} has been withdrawn from account ${self.acc_num}. Remaining balance is ${self.balance}.")

    # Check balance
    def check_balance(self):
        print(f"Your balance for account ${self.acc_num} is: {self.balance}")
